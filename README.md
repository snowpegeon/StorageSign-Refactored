...
| 機能 | 説明 | 設定キー |
|---|---|---|
| ホッパー自動インポート | ホッパーがアイテムを押し込むと StorageSign 経由でコンテナへ自動格納 | `auto-import` |
| ホッパー自動エクスポート | ホッパーがアイテムを引き出すと StorageSign 経由でコンテナから自動排出 | `auto-export` |
...
| `auto-import` | `true` | ホッパー自動インポートの有効化 |
| `auto-export` | `true` | ホッパー自動エクスポートの有効化 |
