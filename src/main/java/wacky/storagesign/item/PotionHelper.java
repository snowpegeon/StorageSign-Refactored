...
/**
 * Utility methods for potion items ({@link Material#POTION}, {@link Material#SPLASH_POTION},
 * {@link Material#LINGERING_POTION}).
 *
 * <h3>Data format (backward-compatible)</h3>
 * <ul>
 *   <li><b>Sign text (line 1):</b> {@code "[P|S|L]POTION:SHORT:CODE}
 *       e.g. {@code "POTION:HEAL:0"}, {@code "SPOTION:REGEN:1"}</li>
 *   <li><b>Item lore:</b> {@code "POTION:HEALING:0 100"}
 *       (full PotionType name, for new data)</li>
 * </ul>
 *
 * <h3>Enhance codes</h3>
 * <ul>
 *   <li>{@code "0"} — normal</li>
 *   <li>{@code "1"} — extended ({@code LONG_…})</li>
 *   <li>{@code "2"} — strong ({@code STRONG_…})</li>
 * </ul>
 *
 * <p>Potions outside the special-case list are handled generically: their short name is the first
 * 5 characters of the base type name (after stripping {@code LONG_} / {@code STRONG_}).
 * When Minecraft adds new potion types, they are handled automatically by this generic path.
 */
public final class PotionHelper {
    ...
    public static PotionType fromSignText(String shortName, String code) {
        // Special-case lookup first
        Map<String, PotionType> codeMap = SIGN_LOOKUP.get(shortName);
        if (codeMap != null) {
